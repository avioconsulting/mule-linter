...
import com.avioconsulting.mule.linter.extension.components.HttpComponent
...
class HttpTests extends Specification {
    ...
    def 'Get Typed HTTP Listener Component'() {
        given:
        app

        when:
        def httpComponents = app.findComponents("listener", Namespace.HTTP)

        then:
        httpComponents.size() == 1
        httpComponents.get(0) instanceof HttpComponent
        httpComponents.get(0).path == "/api/*"
    }


    def 'HTTP Listener Path Rule Success'() {
        given:
        app
        HttpListenerPathRule rule = new HttpListenerPathRule()
    ...
    def 'HTTP Listener Path Rule Fail'() {
        given:
        testApp.addFile('src/main/mule/main.xml', BAD_API_PATH)
        app = new MuleApplication(testApp.appDir)
        HttpListenerPathRule rule = new HttpListenerPathRule()
        ...
        then:
        violations.size() == 1
        violations[0].fileName.endsWith('main.xml')
    }
...
\t</flow>
    ...
    private static final String BAD_API_PATH = '''
<mule xmlns="http://www.mulesoft.org/schema/mule/core" 
xmlns:apikit="http://www.mulesoft.org/schema/mule/mule-apikit" 
xmlns:doc="http://www.mulesoft.org/schema/mule/documentation" 
xmlns:ee="http://www.mulesoft.org/schema/mule/ee/core" 
