 ...
 */
public class MuleJsonParserCharArray extends BaseJsonParser {
    ...
    private int skipWhiteSpaceFast(char[] array, int index) {
        char c;
    ...
    private JsonNumber decodeNumber() {
        ...
        return new JsonNumber(num, lineNumber);
    }

    protected static final char[] NULL = Chr.chars("null");

    protected final JsonNull decodeNull() {
        if (__index + NULL.length <= charArray.length) {
            if (charArray[__index] == 'n' &&
                    ...
                    charArray[++__index] == 'l') {
                __index++;
                return new JsonNull(lineNumber);
            }
        }
    ...
    protected final JsonBoolean decodeTrue() {
        if (__index + TRUE.length <= charArray.length) {
            if (charArray[__index] == 't' &&
                ...
                __index++;
                return new JsonBoolean(true, lineNumber);
            }
        }
    ...
    protected final JsonBoolean decodeFalse() {
        if (__index + FALSE.length <= charArray.length) {
            if (charArray[__index] == 'f' &&
                    ...
                    charArray[++__index] == 'e') {
                __index++;
                return new JsonBoolean(false, lineNumber);
            }
        }
    ...
    private JsonString decodeString() {
        ...
        __index = index;
        return new JsonString(value, lineNumber);
    }

    protected final List decodeJsonArray() {
        JsonArray<Object> list = null;
