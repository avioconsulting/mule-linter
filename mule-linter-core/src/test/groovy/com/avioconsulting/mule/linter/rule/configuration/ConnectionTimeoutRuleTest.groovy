...
class ConnectionTimeoutRuleTest extends Specification{
    ...
    def 'HTTP Request with timeout success'() {
        given:
        ...
        Rule rule = new ConnectionTimeoutRule()
        rule.init()
    ...
    def 'HTTP Request-config with timeout configured at config-reference success'() {
        given:
        testApp.addFile('src/main/mule/global-config.xml', GLOBAL_CONFIG)
    ...
    def 'HTTP Request-config without timeout failure'() {
        given:
    ...
    def 'Implementation with more components to check for retry configuration success '() {
        ...
        List components = [[name: 'request', namespace: Namespace.HTTP, 'config-ref': 'request-config'],
                           [name: 'publish', namespace: Namespace.VM, timeoutAttribute: 'responseTimeout', 'config-ref': 'config'],
                           [name: 'publish-consume', namespace: Namespace.VM, timeoutAttribute: 'responseTimeout', 'config-ref': 'config']]
        app = new MuleApplication(testApp.appDir)
        Rule rule = new ConnectionTimeoutRule()
    ...
    def 'Implementation with more components to check for retry configuration failure'() {
        ...
        testApp.addFile('src/main/mule/global-config.xml', GLOBAL_CONFIG_WITHOUT_TIMEOUT)
        testApp.buildConfigContent('http-implementation.xml', IMPLEMENTATION_WITHOUT_TIMEOUT)
        List components = [[name: 'request', namespace: Namespace.HTTP, timeoutAttribute: 'responseTimeout', 'config-ref': 'request-config'],
                           [name: 'publish', namespace: Namespace.VM, timeoutAttribute: 'responseTimeout', 'config-ref': 'config'],
                           [name: 'publish-consume', namespace: Namespace.VM, timeoutAttribute: 'responseTimeout', 'config-ref': 'config']]
        app = new MuleApplication(testApp.appDir)
        Rule rule = new ConnectionTimeoutRule()
        ...
        violations.size() == 3
        violations[0].message == 'Connector timeout is not configured for component: request'
        violations[1].message == 'Connector timeout is not configured for component: publish'
        violations[2].message == 'Connector timeout is not configured for component: publish-consume'
    }
...
'''
    private static final String GLOBAL_CONFIG_WITHOUT_TIMEOUT = '''
<mule xmlns:topgolf-core="http://www.mulesoft.org/schema/mule/topgolf-core" xmlns:api-gateway="http://www.mulesoft.org/schema/mule/api-gateway"
\txmlns:vm="http://www.mulesoft.org/schema/mule/vm"
\txmlns:secure-properties="http://www.mulesoft.org/schema/mule/secure-properties"
\txmlns="http://www.mulesoft.org/schema/mule/core"
...
'''
    private static final String IMPLEMENTATION_WITH_TIMEOUT = '''
...
\t</sub-flow>
    ...
    private static final String IMPLEMENTATION_WITHOUT_TIMEOUT = '''
<mule xmlns:vm="http://www.mulesoft.org/schema/mule/vm" xmlns:http="http://www.mulesoft.org/schema/mule/http" xmlns:ee="http://www.mulesoft.org/schema/mule/ee/core"
\txmlns:topgolf-core="http://www.mulesoft.org/schema/mule/topgolf-core"
\txmlns="http://www.mulesoft.org/schema/mule/core" xmlns:doc="http://www.mulesoft.org/schema/mule/documentation" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xsi:schemaLocation="http://www.mulesoft.org/schema/mule/core http://www.mulesoft.org/schema/mule/core/current/mule.xsd
http://www.mulesoft.org/schema/mule/topgolf-core http://www.mulesoft.org/schema/mule/topgolf-core/current/mule-topgolf-core.xsd
