...
class CronExpressionExternalizedRuleTest extends Specification{
    ...
    def 'Cron Expression correctly configured within the property files '() {
        ...
        Rule rule = new CronExpressionExternalizedRule()

        when:
        Application app = new MuleApplication(testApp.appDir)
    ...
    def 'Cron Expression Hardcoded in the Mule Configuration file'() {
        ...
        then:
        violations.size() == 2
        violations[0].message == CronExpressionExternalizedRule.CRON_EXPRESSION_HARD_CODED+"sales-orders-csv-impl.xml"+". The hard coded cron expression is : * * 10 * * * *"
        violations[0].lineNumber == 11

        violations[1].message == CronExpressionExternalizedRule.CRON_EXPRESSION_HARD_CODED+"sales-orders-csv-impl.xml"+". The hard coded cron expression is : * * 12 * * * *"
        violations[1].lineNumber == 82
    }
...
\t\t\t<scheduling-strategy >
\t\t\t\t<cron expression="${cron.file.listener.expression}" />
\t\t\t</scheduling-strategy>
...
\t\t\t<scheduling-strategy >
\t\t\t\t<cron expression="${cron.scheduler.expression}" timeZone="CST"/>
\t\t\t</scheduling-strategy>
\t\t</scheduler>
\t</flow>
</mule>
